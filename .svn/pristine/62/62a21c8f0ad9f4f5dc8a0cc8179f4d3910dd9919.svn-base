...
/**
 * 
 * ClassName: PubZzjgsyController
 * Function: TODO 组织机构使用维护. 
 * Reason: TODO ADD REASON(可选).
 * date: 2017年4月7日 上午9:19:38
 * 
 * @author wei
 */
@Controller
...
public class PubZzjgsyController {
	...
	private TreeNode pubZzjgBo2TreeNode(PubZzjgBo pubResBo) {
		if (null != pubResBo) {
			TreeNode treeNode = new TreeNode();
			treeNode.setId(pubResBo.getDwdm() + "");
			treeNode.setText(pubResBo.getJhdwmc());
			TreeNodeState state = new TreeNodeState();
			if (pubResBo != null && !StringUtils.isEmpty(pubResBo.getGxdwdm())) {
				treeNode.setParent(pubResBo.getGxdwdm());
...
//			treeNode.setOrder(pubResBo.getLevel());
	...
	public String getSelectedOrgTree2(String zdbh) throws Exception{
		...
		List<PubZzjgBo> pubZzjgBos = pubZzjgsyService.queryPubZzjgBoByZdbh(zdbh, 2);
		List<TreeNode> nodeList = new ArrayList<TreeNode>();
		if(pubZzjgBos != null){
			for(PubZzjgBo pubZzjgBo : pubZzjgBos){
				nodeList.add(pubZzjgBo2TreeNode(pubZzjgBo));
