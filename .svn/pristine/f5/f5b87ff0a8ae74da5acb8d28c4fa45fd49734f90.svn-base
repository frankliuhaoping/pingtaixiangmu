    ...
    <div class="wrapper wrapper-content">
        <div class="ibox float-e-margins">
            <div class="ibox-title">
                <h5><spring:message code="pubParam.select" /></h5>
            </div>
...
$(function () {
    ...
    $("#addBtn").click(function(){
    	indexPage = Layer.openIframeByOptions(pubParamUrl.add,{});
    });
...
function cszFormatter(value, row) {
    if(value == null || value.trim() == ""){
    	...
    	//这一段是请求的参数返回后进行字符串处理，长度如果大于20则截取0-20位，后面以...补充。
    	/* if (value.length > 20) {
...
// 修改全局参数
function edit(rowId) {
	indexPage = Layer.openIframeByOptions(pubParamUrl.edit + rowId,{});
}
...
// 查看全局参数
function view(rowId) {
	indexPage = Layer.openIframeByOptions(pubParamUrl.view + rowId,{});
