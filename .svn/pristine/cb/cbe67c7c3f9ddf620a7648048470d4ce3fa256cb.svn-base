...
<%@page import="com.htcf.frame.base.util.ParamHelper"%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib prefix='c' uri='http://java.sun.com/jsp/jstl/core'%>
<%@ taglib prefix="spring" uri="http://www.springframework.org/tags"%>
<%@ taglib prefix="shiro" uri="http://shiro.apache.org/tags" %>
<%
	String pkiLoginUrl = ParamHelper.getValue("pki_login_url");

	//pkiLoginUrl = "https://10.235.180.109/web-test/pkiLogin?fallbackUrl=" + fallbackUrl;
	//pkiLoginUrl = "https://10.235.180.109/web-test/pkiLogin";
	//System.out.println(pkiLoginUrl);
	
	String errorMsg = request.getParameter("errorMsg");
	if( StringUtils.isNotEmpty( errorMsg ) )
		request.setAttribute("error", URLDecoder.decode(errorMsg, "UTF-8"));
%>
<!DOCTYPE html>
<html>
    ...
    <div class="loginbody">
    	...
    	<form id="loginForm"  action="" method="post">
		    <ul>
			    ...
			    <li><button id="loginBtn" type="submit" class="loginbtn">登    录</button>&nbsp;<input id="pkiLoginButton" name="" type="button" class="loginbtn2" value="PKI登录" /></li>			    
                    <li><span style="color:red; font-size:10px;" id="errorText">
...
<%--         	$.ajax({
            ...
            var pkiLoginUrl = "<%=pkiLoginUrl%>";
            var url = window.location.href;
            
            
            var b = url.indexOf("errorMsg=");
            if( b != -1 )
            {
            	  var e = url.indexOf(b,"&");
                  ...
                  var errorMsg = url.substring(b, e);
            ...
            pkiLoginUrl += "?fallbackUrl=" + encodeURIComponent(url);
