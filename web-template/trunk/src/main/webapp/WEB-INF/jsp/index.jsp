        ...
        <c:forEach items="${pubAppList}" var="pubApp">
            <div class="form-group col-sm-3">
                <div class="form-group col-sm-12" style="text-align:center;">
                   <img title="${pubApp.yymc}" src="${commonResUrl}/common/static/img/index/64px/${pubApp.yydm}.png" onclick="openSystem('${pubApp.fwlj}', '${pubApp.yydm}')">
                </div>
...
// ***********************websocket 使用js,需要用websocket的放开注释***********************
/*
var echo_websocket;
if ("WebSocket" in window) {
    echo_websocket = new WebSocket("ws://${socketAddress}/websocket?yydm=${yydm}");
} else if ("MozWebSocket" in window) {
    echo_websocket = new MozWebSocket("ws://${socketAddress}/websocket?yydm=${yydm}");
} else {
    echo_websocket = new SockJS("http://${socketAddress}/websocket/sockjs?yydm=${yydm}");
}
...
echo_websocket.onmessage = function(event) {
    ...
    console.log('Received: ' + JSON.parse(event.data)); //处理服务端返回消息
...
echo_websocket.onclose = function(event) {
    ...
    console.log(event);
    Layer.msg("该账号在其他页面创建socket连接，socket关闭", 'success');
};
//***********************websocket 使用js,需要用websocket的放开注释***********************
*/
</script>
