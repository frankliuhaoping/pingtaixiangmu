...
public class PubResController {
	...
	public String getTreeGrid(HttpServletRequest request) throws Exception {
		...
		String nodeId = (String) (StringUtil.isEmpty(nodeIdStr) ? "0" : nodeIdStr);
		// 修改  可能为0会有问题
		String yyid = StringUtil.isEmpty(yyidStr) ?  "0" : yyidStr;
		...
		for (PubResBo pubResBo : pubResBos) {
			// 应用ID相同
			if (null != pubResBo.getYyid() && pubResBo.getYyid().equals(yyid)) {
				PubResGrid pubResBoGrid = new PubResGrid();
				Mapper mapper = new DozerBeanMapper();
				mapper.map(pubResBo, pubResBoGrid);
				// pubResBo.getZyfid() == 0
				if (StringUtil.isEmpty(pubResBo.getZyfid())) {
	...
	public String edit(@PathVariable String id, HttpServletRequest request) throws Exception {
		logger.entry("-------修改权限");
		// 修改 PubRes pubRes = pubResService.getResByZyid(Long.valueOf(id));
		PubRes pubRes = pubResService.getResByZyid(id);
		//这里要判断资源父ID 不能等于0
		// pubRes.getZyfid() != 0
	...
	public String getTree(HttpServletRequest request) throws Exception {
		...
		ActionResult actionResult = ActionResult.success().build();
		// long id = 0;
		String id = StringUtil.isEmpty(parentId) ? "0" : parentId;
		//判断非等于#的
...
//		pubResBos = reGenerateTree(pubResBos);
		...
		if (pubResBos == null || pubResBos.size() <= 0) {
			return JsonUtil.toJsonNonNull(actionResult);
		}
	...
	private static TreeNode pubRes2TreeNode(PubResBo pubResBo) {
		if (null != pubResBo) {
			...
			treeNode.setText(pubResBo.getZymc());
				//pubResBo.getZyfid() != 0
			if (pubResBo != null && pubResBo.getZyfid() != null && !pubResBo.getZyfid().equals("0")) {
