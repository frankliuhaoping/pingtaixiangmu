...
public class PubXtzdController {
	...
	public String getTreeGrid(HttpServletRequest request) throws Exception {
		...
		String yyid = StringUtils.isEmpty(yyidStr) ? "0" : yyidStr;
		int level = StringUtils.isEmpty(levelStr) ? 0 : Integer.valueOf(levelStr);
		
		   //根据应用ID，字典bh获取字典列表
		List<PubXtzdBo> pubXtzdBos = pubXtzdService.getPubXtzdNodesById(yyid, nodeId, 0);
		List<PubXtzdGrid> grid = new ArrayList<PubXtzdGrid>();
		for (PubXtzdBo pubXtzdBo : pubXtzdBos) {
			// 应用ID相同
			if (null != pubXtzdBo.getYyid() && pubXtzdBo.getYyid().equals(yyid)) {
				...
				if (null == pubXtzdBo.getZdfbh() || "".equals(pubXtzdBo.getZdfbh())) {
	...
	@ResponseBody
	public String selectGrid(HttpServletRequest request) throws Exception{
		  logger.entry("-------查询系统字典树表格数据");
		 Map<String,Object> map = WebUtils.getParametersStartingWith(request, null);
		
		 //应用id
		 String yyid = null == map.get("yyid") ? "0" : map.get("yyid").toString();
		 //父编号
		 String nodeId = null == map.get("nodeid") ? null : map.get("nodeid").toString();
		 //等级
		 int level = null == map.get("n_level") || "" .equals(map.get("n_level")) ? 0 : Integer.valueOf(map.get("n_level").toString());
		 //字典项名
		 String zdxm = null == map.get("zdxm") ? null : map.get("zdxm").toString();
		 //字典项值
		 String zdxz = null == map.get("zdxz")? null : map.get("zdxz").toString();				
		 //字典编号
		 String zdbh = null == map.get("zdbh") ? null : map.get("zdbh").toString();
		 
		 //根据应用ID，字典bh获取字典列表
		 List<PubXtzdBo> pubXtzdBos = pubXtzdService.selectPubXtzdNodesById(yyid, nodeId, zdbh, zdxm, zdxz);
		 List<PubXtzdGrid> grid = new ArrayList<PubXtzdGrid>();
		 Mapper mapper = new DozerBeanMapper();
		 //计算等级节点
	     for (PubXtzdBo pubXtzdBo : pubXtzdBos) {
	    	 ...
	    	 //首先判断顶级节点
	    	 if (StringUtils.isBlank(pubXtzdGrid.getZdfbh())) {
	    		 pubXtzdGrid.setLevel(0);
	    		 pubXtzdGrid.setIsLeaf(false);
	    		 pubXtzdGrid.setExpanded(true);
	    		 pubXtzdGrid.setParent(null);
	    		 pubXtzdGrid.setId(pubXtzdGrid.getBh() + "");
	    	 } else {
	    		 PubXtzd pubxtzd =pubXtzdService.getPubXtzdByBh(pubXtzdGrid.getZdfbh());
	    	    //判断子集
	    	    if (StringUtils.isBlank(pubxtzd.getZdfbh())) {
	    	    	if (pubXtzdService.getCountByZdfbh(yyid,pubXtzdGrid.getBh()) > 0) {
	    	    		 pubXtzdGrid.setLevel(1);
	    	      		 pubXtzdGrid.setIsLeaf(false);
	    	      		 pubXtzdGrid.setExpanded(true);
	    	      		 pubXtzdGrid.setParent(pubXtzdGrid.getZdfbh()+ "");
	    	      		 pubXtzdGrid.setId(pubXtzdGrid.getBh() + "");
	    	    	    } else {
	    	    	    	 pubXtzdGrid.setLevel(1);
	    	         		 pubXtzdGrid.setIsLeaf(true);
	    	         		 pubXtzdGrid.setExpanded(false);
	    	         		 pubXtzdGrid.setParent(pubXtzdGrid.getZdfbh()+ "");
	    	         		 pubXtzdGrid.setId(pubXtzdGrid.getBh() + "");
	    	    	    }
	    	    } else {
	    	    	PubXtzd pubxtzd1 =pubXtzdService.getPubXtzdByBh(pubxtzd.getZdfbh());
	    	    	if (StringUtils.isBlank(pubxtzd1.getZdfbh())) {
	    	    		if (pubXtzdService.getCountByZdfbh(yyid,pubXtzdGrid.getBh()) > 0) {
	    	    			pubXtzdGrid.setLevel(2);
	    	    			pubXtzdGrid.setIsLeaf(false);
	    	    			pubXtzdGrid.setExpanded(true);
	    	    			pubXtzdGrid.setParent(pubXtzdGrid.getZdfbh()+ "");
	    	    			pubXtzdGrid.setId(pubXtzdGrid.getBh() + "");
	     ...
	     Collections.sort(grid);
	     List<PubXtzdGrid> grid1 = new ArrayList<PubXtzdGrid>();
	     mapper.map(grid, grid1);
	     //子集位置更改，无法排序
	     for (PubXtzdGrid pubXtzdGrid2 : grid) {
	    	 
			if (StringUtils.isNotBlank(pubXtzdGrid2.getZdfbh())) {
				grid1.remove(pubXtzdGrid2);
				PubXtzd pubxtzd =pubXtzdService.getPubXtzdByBh(pubXtzdGrid2.getZdfbh());
				int i = grid1.indexOf(pubxtzd);
				grid1.add(i+1,pubXtzdGrid2);
			}
		 }
		 return JsonUtil.toJsonNonNull(grid1);
	}
...
//		}
		// 按树型结构返回字典
//		List<PubXtzdBo> pubXtzdBos = pubXtzdService.getPubXtzdNodesById(yyid, parentId, 1);
		// 重新生成树，因为本身菜单不一定是叶子节点，并把树型变成并列数据
//		pubXtzdBos = reGenerateTree(pubXtzdBos);
		// 按并列结构返回字典
		List<PubXtzdBo> pubXtzdBos = pubXtzdService.getPubXtzdNodesById(yyid, parentId, 2);
	...
	public String getPubXtzdList(HttpServletRequest request) throws Exception {
		...
		List<PubXtzd> pubXtzds = pubXtzdService.getPubXtzdListByZdbh(pubXtzd.getYyid(), zdbh);
		ar.setData(pubXtzds);
		return JsonUtil.toJsonNonNull(ar);
	}
