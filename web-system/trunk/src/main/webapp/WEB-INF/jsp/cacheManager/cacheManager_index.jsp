       ...
       <div class="col-sm-2" style="padding-left:0;padding-right:0;">
         <div class="ibox float-e-margins">
            <div class="ibox-title">
               <h5><spring:message code="cacheManager.tree"/></h5>
            </div>
...
var cacheManagerUrl = {
    ...
    getListTree:"<c:url value='/cacheManager/getListTree' />"
};
//初始化表格和按钮
$(function () {
	//初始化左边的列表树
    ...
    $("#refreshBtn").click(function(){
        ...
        ManageGrid.refreshTable(table, cacheManagerUrl.getData,param);
    });
    // 批量删除缓存
    $("#removeBtn").click(function(){
    	var zdbh =$.trim($("#zdbh").val());
        ManageGrid.deleteMultiple(table, cacheManagerUrl.removeBatch+"?zdbh="+zdbh, function(){window.location.href = cacheManagerUrl.index + "?limit=" + limit + "&offset=" + offset;},"cacheName");
    });
...
function initTree()
{
	 $("#appTree").jstree({
	        ...
	        'core' : {
	            'data' : {
	                "url" : cacheManagerUrl.getListTree,
	                "data" : function (node) {
...
function initTable()
{
	  //获取字典的编号
	    table.bootstrapTable({
	        url: cacheManagerUrl.getData + "?zdbh="+selectTreeId,
...
function view(cacheName) {
	viewCacheName = cacheName;
	var zdbh =$.trim($("#zdbh").val());
	indexPage = Layer.openIframeByOptions(cacheManagerUrl.view + cacheName+"?zdbh="+zdbh+"&cacheName="+cacheName,{});
}

//删除缓存
function del(rowId) {
	var zdbh =$.trim($("#zdbh").val());
    ManageGrid.deleteSingle(cacheManagerUrl.remove + rowId + "?zdbh=" + zdbh+"&cacheName="+rowId, function(){window.location.href = cacheManagerUrl.index + "?limit=" + limit + "&offset=" + offset;});
}
