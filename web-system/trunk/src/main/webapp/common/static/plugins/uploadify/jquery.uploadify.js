...
(function ($) {
    ...
    var handlers = {
        ...
        onDialogOpen: function () {
            ...
            this.queueData.errorMsg = '某些文件未添加到上传队列中:';
        ...
        <div><div class="fileName" title="${fileName}">${fileName}</div><div>${fileSize}<span class="data"></span></div></div>\
                ...
                switch (errorCode) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (settings.queueSizeLimit > errorMsg) {
                            this.queueData.errorMsg += '\n所选文件数超出剩余可上传数量限制(' + errorMsg + ').';
                        } else {
                            this.queueData.errorMsg += '\n所选文件数超出最大可上传数量限制(' + settings.queueSizeLimit + ').';
                        }
                        break;
                    ...
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        ...
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件"' + file.name + '"是不允许的文件类型(' + settings.fileTypeDesc + ').';
                        break;
                }
            }
        ...
        onUploadError: function (file, errorCode, errorMsg) {
            ...
            if ($.inArray('onUploadError', settings.overrideEvents) < 0) {
                ...
                if (errorCode != SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND && file.status != SWFUpload.FILE_STATUS.COMPLETE) {
                	$('#' + file.id).find('.data').removeClass('blue').addClass('red');
        ...
        onUploadProgress: function (file, fileBytesLoaded, fileTotalBytes) {
            ...
            if ($.inArray('onUploadProgress', settings.overrideEvents) < 0) {
                if (settings.progressData == 'percentage') {
                	$('#' + file.id).find('.data').addClass('blue');
        ...
        onUploadSuccess: function (file, data, response) {
            ...
            if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
            	$('#' + file.id).find('.data').removeClass('blue').addClass('green');
