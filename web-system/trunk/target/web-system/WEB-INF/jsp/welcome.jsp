    	...
    	<div >
    		<div class="indexlogo clearfix">
    			...
    			<p class="logotxt">公安指挥与控制系统</p>
    				...
    				<a href="/web-system/logout" class="headclose pull-right"><i class="fa fa fa-sign-out" style="margin-right:2px;"></i>&nbsp;退出</a>
    			
    		</div>
    	</div>
    </div>
    <div style="background:#0059a5;height:25px;width:100%;"></div>
    <div style="width:100%;min-width:1280px;overflow:auto;background:url(<c:url value='/common/static/img/content_bg.png' />) no-repeat center center / 100%;position:relative;" class="index-center">
    	...
    	<div class="main_desktop clearfix">
			...
			<ul class="desktop_wrap clearfix pull-left">
			  <c:forEach items="${pubAppList}" var="pubApp" varStatus="status">
			  	...
			  	<p>${pubApp.yymc}</p>
		        <a href="#"><img title="${pubApp.yymc}" src="${commonResUrl}/common/static/img/index/64px/${pubApp.yydm}.png" height="64" width="64" onclick="openSystem('${pubApp.fwlj}', '${pubApp.yydm}')"></a></li>
		      	</li>
		      </c:forEach>
			</ul>
			<div class="desktopbtn toright pull-right" onclick="scrollSection(0, false)"><i class="fa fa-angle-right"></i></div>
		  	<div id="switch">
			   <div id="switch-content">
			      <div id="toolbar" class="navigation">
			         <c:forEach var="i" begin="1" end="${pageNumber}" varStatus="status">
			             <a id="tab-${status.index}" onclick="scrollSection(${status.index})"
...
function scrollSection (liIndex, leftFlag) {
	...
	//兄弟元素去除样式
	selectedTab.siblings().removeClass();
