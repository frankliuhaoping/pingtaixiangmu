...
<div class="wrapper wrapper-content">
            ...
            <div class="ibox-content pb10 clearfix">
				 ...
				 <!-- 选择和查看tab选项卡  -->
				 	<ul id="myTab" class="nav nav-tabs" role="tablist">
						...
						<li >
							<a href="#copyPubJqxzsy" data-toggle="tab">导入</a>
						</li>
					</ul>
				 	...
				 	<div id="myTabContent" class="tab-content">
						...
						<div id="copyPubJqxzsy" class="tab-pane fade " style="border: #ddd solid 1px;margin:0; padding: 10px 0;overflow:auto;" >		
			                      <!-- 下拉选择 -->
    ...
    <form action="" id="saveForm">
    	...
    	<input type="hidden" name="jqxzbh" id="jqxzbh" />
    </form>
...
//url定义
var pubJqxzsyUrl = {
    add: '<c:url value="/pubJqxzsy/saveAdd" />',
    copy:'<c:url value="/pubJqxzsy/copy" />',
    getOrgTreeView : "<c:url value='/pubJqxzsy/getOrgTreeView'/>",
    //获取选中树
    loadSelectedTree: "<c:url value='/pubJqxzsy/getSelectedOrgTree/' />"
};
//table对象
...
function registerActionEvent(){
	 ...
	 $('#saveBtn').click(function(){
		 //得到所有选中节点数组,存入jqxzbh
	   	 $("[name='jqxzbh']").val(authTree.jstree('get_selected').join());
	  	 Manage.save(pubJqxzsyUrl.add, $('#saveForm').serialize(), function(){
	  		 //保存完毕之后刷新查看警情性质使用查看
	 	  	getViewTree();
	  	 });
	 });
...
function initTree(){
	//系统字典树
	 $("#appTree").jstree({
	        ...
	        'core' : {
	            'data' : {
	                "url" : "<c:url value='/pubJqxzsy/getOrgUseTree' />",
	                "data" : function (node) {
	...
	var url = "<c:url value='/pubJqxzsy/getOrgTree' />";
...
function loadSelectedTree(){
	...
	Manage.query (pubJqxzsyUrl.loadSelectedTree, {zdbh:selectTreeId}, function (result){
		if(!authTree){
			return ;
		}
...
// 	viewTree.data('jstree', false).empty(); 
	Manage.query (pubJqxzsyUrl.getOrgTreeView, {zdbh:selectTreeId}, function (result){
		viewTree.jstree({
			'core': {
				'multiple': true,
				'dblclick_toggle':false,
...
$('#copy').click(function(){
	var zdbh=$('#zdbh').val();
	var bh=$('#bh').val();
  	 Manage.save(pubJqxzsyUrl.copy,{zdbh:zdbh,bh:bh});
