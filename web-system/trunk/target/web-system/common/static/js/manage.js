        ...
        var htcfAjaxStatus = XMLHttpRequest.getResponseHeader("htcfAjaxStatus"); 
            if("timeout" == htcfAjaxStatus){//处理shiro session超时
        	    ...
        	    window.setTimeout(function () { 
        		    //跳转的登录页面
	                window.top.location.href = document.URL.split("/")[0] + "//" + document.URL.split("/")[2] + "/" + document.URL.split("/")[3] + "/index";
        	    }, 3000);
 ...
 * @param msgOpts  操作完成后用于提示消息的json对象，根据服务端返回标记来填充
 */
ManageGrid.synchroBatch = function(table, url, callback,idKey, msgOpts){
	var title = msgOpts && msgOpts.confirmTitle ? msgOpts.confirmTitle : '同步确认';
	var msg = msgOpts && msgOpts.confirmMsg ? msgOpts.confirmMsg : '您确认要同步吗';
	var rows = table.bootstrapTable('getSelections');
	if(rows.length > 0){
		createConfirmDialog(title, msg+rows.length+'条记录吗？', function(){
			var idArr = $.map(rows, function (row) {
...
Layer.openIframeByOptions = function(iframeSrc, options) {
	Layer.checkLayer();
	var index = layer.open({
		type: 2,
		//shadeClose: true,
		shade: options.shade!=undefined ? options.shade : 0,
		title: options.title!=undefined ? options.title : false,
		area: options.area!=undefined ? options.area : ['0px', '0px'],
		content: iframeSrc,
		closeBtn: options.closeBtn!=undefined ? options.closeBtn : 0
	});
	if (options.maxFlag==undefined) {
		layer.full(index);
	}
	return index;
}
